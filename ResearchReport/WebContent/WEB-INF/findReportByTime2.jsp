...
<body>
	<div>
	<form action="${pageAction.request.actionPath}/FindReportAction_findReportByTime" method="post">
		<table>
			<tr>
				<td>查询科研项目</td>
				<td><input type="text" name="end_time" placeholder="请输入查询的年份"></td>
				<td>
					<select name="semester">
						<option value="all" <s:if test="semester='all'">selected="selected"</s:if>>全部</option>
						<option value="first" <s:if test="semester='first'">selected="selected"</s:if>>第一学期</option>
						<option value="second" <s:if test="semester='second'">selected="selected"</s:if>>第二学期</option>
					</select>
				</td>
